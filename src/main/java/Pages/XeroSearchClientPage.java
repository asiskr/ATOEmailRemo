...
//						ClientExcel.writeCombinedDataToExcel(clientCodeText, subject);
	...
	private boolean searchByRemovingLastCharacter(String clientName) throws InterruptedException {
		while (clientName.length() > 0) {
			clientName = clientName.substring(0, clientName.length() - 1); // Remove the last character
			inputBox.clear();
			inputBox.sendKeys(clientName);
			Thread.sleep(3000);

			try {
				List<WebElement> elements = DriverManager.getDriver().findElements(By.xpath("//a"));
				for (WebElement ele : elements) {
					if (ele.getText().trim().equalsIgnoreCase(clientName.trim())) {
						ele.click();
	...
	public boolean retryWithModifiedClientName(String originalClient) throws InterruptedException {
		...
		// Retry by removing special characters or last character
		...
		// Retry by removing the last character if still not found
